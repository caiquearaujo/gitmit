...
def commit_from_resume_message(
    ...
    contents = [
        ...
        "Please follow rules:",
        "1. You must use an existing commit type from the JSON 'type' property below. To choose the commit type, you should see the resume below and choose the most appropriate commit type that covers the changes. See the 'meaning' property to understand the commit type;",
...
def commit_message(repo: git.Repo, explanation: str = None) -> Optional[str]:
    ...
    contents = [
        ...
        "Please follow rules:",
        "0. When analyzing the changes, new files (aka untracked files) has priority over modified files;",
        "1. You must use an existing commit type from the JSON 'type' property below. To choose the commit type, you should see the changes below and choose the most appropriate commit type that covers the changes. See the 'meaning' property to understand the commit type;",
        "2. If you can't choose a commit type or no commit type is suitable, use the 'other' commit type;",
