...
class ConfigTool:
    ...
    def __init__(self, services: Services):
        ...
        self.config_path: Path = Path(services.path)

    def _hide_sensitive(self, value: str, is_sensitive: bool = False) -> str:
        """Hide sensitive data or show empty values properly."""
        if not value or value.strip() == "":
            return "<empty>"
        if is_sensitive and value not in [
            "<your-google-api-key>",
            "<your-openrouter-api-key>",
            "<your-mysql-password>",
        ]:
            return "<hidden>"
        if value in ["<your-google-api-key>", "<your-openrouter-api-key>", "<your-mysql-password>"]:
            return "<not set>"
        return value

    def _get_token_usage(self) -> tuple[int | None, str | None]:
    ...
    def run(self):
        ...
        lines.append(
            f"  Permissions: [bold yellow]{oct(self.config_path.stat().st_mode)[-3:]}[/bold yellow]"
        )
        lines.append("")
        ...
        lines.append(
            f"  Commit Model: [bold yellow]{self._hide_sensitive(commit_model)}[/bold yellow]"
        )
        lines.append(
            f"  Resume Model: [bold yellow]{self._hide_sensitive(resume_model)}[/bold yellow]"
        )
        ...
        if token_usage is not None and active_model is not None:
            ...
            lines.append(
                f"  Current Month Tokens: [bold yellow]{token_usage:,}[/bold yellow]"
            )
        lines.append("")
        ...
        lines.append(
            f"  API Key: [bold yellow]{self._hide_sensitive(google_api_key, is_sensitive=True)}[/bold yellow]"
        )
        lines.append("")
        ...
        lines.append(
            f"  Host: [bold yellow]{self._hide_sensitive(ollama_host)}[/bold yellow]"
        )
        lines.append("")

        # OpenRouter section
        openrouter_api_key = config.get("openrouter", "api_key", fallback="")
        openrouter_providers = config.get("openrouter", "providers", fallback="")
        lines.append("[bold cyan]OpenRouter:[/bold cyan]")
        lines.append(
            f"  API Key: [bold yellow]{self._hide_sensitive(openrouter_api_key, is_sensitive=True)}[/bold yellow]"
        )
        lines.append(
            f"  Providers: [bold yellow]{self._hide_sensitive(openrouter_providers)}[/bold yellow]"
        )
        lines.append("")
        ...
        lines.append(
            f"  Host: [bold yellow]{self._hide_sensitive(mysql_host)}[/bold yellow]"
        )
        lines.append(
            f"  Port: [bold yellow]{self._hide_sensitive(mysql_port)}[/bold yellow]"
        )
        lines.append(
            f"  User: [bold yellow]{self._hide_sensitive(mysql_user)}[/bold yellow]"
        )
        lines.append(
            f"  Password: [bold yellow]{self._hide_sensitive(mysql_password, is_sensitive=True)}[/bold yellow]"
        )
        lines.append(
            f"  Database: [bold yellow]{self._hide_sensitive(mysql_database)}[/bold yellow]"
        )
        ...
        if self.services.database:
            try:
                if not self.services.database.connected:
                    self.services.database.start()
                db_status = (
                    "Connected" if self.services.database.connected else "Failed"
                )
            except Exception:
